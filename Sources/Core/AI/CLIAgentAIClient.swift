...
/// subscription. OpenTodo ships unsandboxed and ad-hoc signed, so spawning the
/// tool is permitted.
///
...
public struct CLIAgentAIClient: AIClient {
    ...
    private func runCodex(system: String, user: String) async throws -> String {
        ...
        let lastMessageURL = scratch.appendingPathComponent("opentodo-codex-\(UUID().uuidString).txt")
...
enum CLIProcess {
    static func run(
        ...
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<CLIProcessResult, Error>) in
            let queue = DispatchQueue(label: "com.opentodo.cli-agent", qos: .userInitiated)
            queue.async {
                do {
                ...
                let readQueue = DispatchQueue(label: "com.opentodo.cli-agent.read", attributes: .concurrent)
