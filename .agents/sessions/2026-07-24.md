...
## Session 5 — Rebase onto board/privacy, full review pass (Opus 5)

Continuation of Session 4. The branch was sitting on a stale base, so the first job
was rebasing onto merged work without reverting it; then a full review of the
implementation — correctness in the running app, DRY, optimization, bugs.

### System flow

```mermaid
flowchart TD
    A[SmartList.filter — single predicate] --> B[MainTabView badge: now: today]
    A --> C[TaskListContainer list: includeCompleted false]
    A --> D[TaskListContainer board: includeCompleted true]

    E[.newTask ⌘N] --> F[TaskListContainer.body .onReceive]
    F --> G[presentQuickAdd — works on Completed too]
```

### What was done

- [x] Rebased onto `origin/master` (`bf08aab`), hand-resolving
      `TaskListContainer.swift` so kanban board (#74), privacy manifest (#78) and
      roadmap (#71) all survive. `git diff origin/master --stat` now touches only
      the 5 intended files.
- [x] **DRY fix:** `SmartList.filter` gained `includeCompleted: Bool = false` so the
      board reuses the one predicate. #74 had re-implemented the smart-list filter
      inline in `TaskListContainer` — two copies of "what's in Today".
- [x] **Bug fix:** `.onReceive(.newTask)` hoisted from `QuickAddChip` into
      `TaskListContainer.body`. The chip is only rendered `if !isCompletedList`, so
      macOS ⌘N was dead on the Completed list.
- [x] Removed a redundant `.accessibilityLabel("Add task")` — the `Label` already
      supplies it. The ✨ zone keeps its label (an `Image` has none).
- [x] Verified interactively in the simulator: quick-add creates a task with parsed
      due date + priority; Today badge and Today list agree; board renders, cards
      drag between columns (`taskService.move`) and the move persists; task and the
      per-selection layout preference both survive terminate → relaunch; `PlanSheet`
      degrades gracefully with no API key ("Add your AI API key in Settings").
- [x] `swiftlint --quiet` clean; iOS build succeeded with no warnings in `Sources/`.

### Key decisions

- **One predicate, two layouts.** `includeCompleted` is a parameter on
  `SmartList.filter` rather than a second function, because the invariant worth
  keeping is "the badge count and the list can never disagree". Placed before `now:`
  so existing call sites keep compiling.
- **`.onReceive` belongs to the pane, not the chip.** Exactly one
  `TaskListContainer` exists per macOS window, so hoisting it can't multi-fire, and
  it keeps working where the chip is hidden.
- **Kept the hand-rolled glass capsule** instead of master's `.buttonStyle(.glassProminent)`.
  The design is one capsule with two tap zones; `.glassProminent` would give two
  separate glass pills, which is the thing the change set out to remove.

### Files changed

| File | Change |
|---|---|
| `Sources/Features/Navigation/SmartList.swift` | `filter` takes `includeCompleted`, so the board shares the predicate |
| `Sources/Features/TaskList/TaskListContainer.swift` | rebase resolution: board + `@AppStorage` layout preserved, chip merge reapplied, `.onReceive(.newTask)` hoisted here |
| `Sources/Features/QuickAdd/QuickAddChip.swift` | dropped the internal `.onReceive` and a redundant accessibility label |

### Mistakes and fixes

- **Stale base.** A PR from the old base would have shown 929 deletions, reverting
  the board, privacy manifest and roadmap. Caught by diffing against
  `origin/master` before pushing. Backup branch: `backup/pre-rebase-b67979d`.
- **False-alarm "persistence bug".** Two simulators were booted, so
  `xcrun simctl … booted` resolved to `OpenTV iPhone 11 Pro` while the interactive
  panel drove `iPhone 17 Pro`. The "vanishing task" and "disappearing layout picker"
  were screenshots of a device that never had the task. **Always pass the explicit
  UDID.** Re-verified on `8CFDD94F-…`: both behaviours are correct.
- Mid-rebase the tree briefly didn't compile — `TaskListContainer` called
  `filter(_:includeCompleted:)` before the parameter existed.

### Known findings, not fixed here

- `TaskListContainer` / `MainTabView` both hold an unpredicated
  `@Query var tasks: [TodoTask]`, so every live tab fetches the whole store and
  `visibleTasks()` re-sorts it on each body evaluation. Fine at current scale;
  wants a predicate + cached sort before the store gets large. Pre-existing.
- `DateExpressionParser.nextWeekday` forces `delta = 7` when a bare weekday name
  matches today, so "report fri 5pm" typed on Friday morning lands next Friday.
  Todoist would use today. Deliberate per the code comment, uncovered by tests,
  and in `Sources/Core` — worth its own issue + tests rather than a UI PR.

### Next steps

- Quick-add still can't route `#project` to a project, and labels aren't surfaced on
  rows — scoped GH issue still pending Vincent's confirm.
