...
Direction and hard facts for OpenTodo. One line per fact.

- **What** — OpenTodo: a native Swift/SwiftUI todo app for macOS 26 + iOS 26, replacing Todoist. Liquid Glass design, iCloud sync, AI planning agent.
- **No monorepo** — pure native app; the GitHub repo is also the marketing page. No server, no JS, no Electron/React Native.
- **Layers** — `TodoCore` (pure engine, no SwiftData) → `TodoData` (SwiftData @Model + @Observable services + CloudKit-ready ModelContainer) → `Sources/{App,Features,SharedUI,Platform}` SwiftUI app + `Sources/CLI` `todo` tool. See ARCHITECTURE.md.
